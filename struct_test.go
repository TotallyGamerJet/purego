...
func TestRegisterFunc_structArgs(t *testing.T) {
	...
	//}
	{
		type TwoUnsafes struct {
			u0, u1 unsafe.Pointer
		}
		var TwoUnsafesFn func(TwoUnsafes) int32
		purego.RegisterLibFunc(&TwoUnsafesFn, lib, "TwoUnsafes")
		u1 := new(int32)
		*u1 = 100
		u2 := new(int32)
		*u2 = -127
		result := TwoUnsafesFn(TwoUnsafes{unsafe.Pointer(u1), unsafe.Pointer(u2)})
		const want = 100 + -127
		if result != want {
			t.Fatalf("FourInt32s returned %d wanted %d", result, want)
		}
